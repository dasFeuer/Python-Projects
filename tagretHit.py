...
def drawTopBar (win, elapsed_time, targets_pressed, misses):
    pygame.draw.rect(win, "grey", (0, 0, WIDTH, TOP_BAR_HEIGHT))
    time_label = LABEL_FONT.render(
        f"Time: {formatTime(elapsed_time)}", 1, "black")
...
def endScreen(win, elapsed_time, targets_pressed, clicks):
    win.fill(BG_COLOR)
    time_label = LABEL_FONT.render(
        f"Time: {formatTime(elapsed_time)}", 1, "white")
...
def main():
    ...
    while run:
        ...
        if misses >= LIVES:
            endScreen(WIN, elapsed_time, target_pressed, clicks)

        draw(WIN, targets)
        drawTopBar(WIN, elapsed_time, target_pressed, misses)
        pygame.display.update()
